...
The curated pipelines we offer gather all the `data` key information from the Release, ReleasePlan, and ReleasePlanAdmission resources.
We then combine them according to the following rules depending on the type of the value:

* *Array type:* The values will be merged together with duplicates removed.

* *Basic type:* The values will be overriden and the priority is as follows:
the information in the ReleasePlan takes precedence over the Release, and the ReleasePlanAdmission overrides them both.
